...
function Waveform({ audio, currentTime, onSeek, audioElement }) {
    ...
    const drawLiveWindow = (time) => {
        ...
        // Show a moving 0.08-second slice of the real signal while it plays.
        // The full waveform remains available when playback is stopped.
        const samplesPerSecond = data.length / duration;
        const windowSeconds = 0.08;
        const windowSamples = Math.max(256, Math.floor(samplesPerSecond * windowSeconds));
    ...
    return (
        <div className="waveform-container">
            <div className="waveform-header">
                <span>{audioElement ? "LIVE TIME DOMAIN" : "TIME DOMAIN"}</span>
            ...
            <div className="waveform-hint">
                {audioElement
                    ? "PLAY THE AUDIO • waveform moves with the actual signal"
